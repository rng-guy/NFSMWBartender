...
* Making Heat transitions very fast (< 5 seconds) can cause a mix of cops from more than one "Event" spawn table (`BartenderSettings\Advanced\Cars.ini`) to appear in events that feature scripted, pre-generated cops. This happens because, depending on your loading times, the game might update the Heat level as it requests those scripted cops. To circumvent this issue, set the `ForceHeatLevel` VltEd parameter of the event in question to the Heat level you are aiming for instead of using short Heat transition timers (wherever possible).
