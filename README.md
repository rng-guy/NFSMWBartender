...
The "Advanced" feature set **also fixes** the displayed engagement count in the centre of the pursuit bar at the bottom of the pursuit HUD: The count now accurately reflects how many chasing cop spawns remain in the current wave by ignoring vehicles that spawn through any Heavy / LeaderStrategy, vehicles that join the pursuit from roadblocks, and the helicopter.
...
Bartender treats all **invalid values** you provide in its configuration files as omitted instead. For comma-separated value tuples, Bartender treats them as omitted if any value in them is invalid. A value is invalid if, for example, you provide a decimal where Bartender expects a whole number. Negative numbers are never invalid: Bartender sets them to 0 instead as it parses a file.
...
To **uninstall** Bartender, remove its files from your game's `scripts` folder. There is no need to remove the optional missing textures, as the game doesn't ever use them without Bartender.
