...
* the **"Advanced" feature set** lets you change [cop-spawning behaviour and tables](https://youtu.be/4NGG1KUmBZQ) without limits.
...
* Challenge Series pursuits now use the free-roam spawning logic for chasing cops,
* the game no longer ignores VltEd settings for roadblocks and Strategies in races,
...
1. read and understand the two sections about [mod (in)compatibilities](#4---which-mods-are-incompatible-with-bartender) and [dependencies](#5---which-mods-does-bartender-depend-on) below,
...
**To update** Bartender, uninstall it and repeat the installation process above. If you update from a version older than v4.00.00, replace all old configuration files.
...
Bartender's **"Advanced" feature set** forces the game to no longer ignore the roadblock-related `pursuitlevels` and the Strategy-related `pursuitsupport` VltEd settings in race pursuits. The feature set also replaces three specific `pursuitlevels` parameters for all Heat levels:
* the `cops` array,
* `HeliFuelTime`, and
