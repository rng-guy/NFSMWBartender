...
* Deleting this file disables the fix for active roadblocks blocking new HeavyStrategy 3 requests, and also the fix for the slowdown of non-Strategy roadblock and Heavy / LeaderStrategy requests.
...
* Bartender fixes the slowdown of non-Strategy roadblock and Heavy / LeaderStrategy requests by clearing the current Strategy request. Bartender does this whenever the game has been stuck trying to spawn a requested Strategy for longer than the Strategy reset timer. Until Bartender clears the stuck Strategy, the game will not request another Strategy. The game will also stop requesting new non-Strategy roadblocks if it happens to be stuck on a HeavyStrategy 4 request.

* Short Strategy reset timers can lead to HeavyStrategy 3 spam. Such spam can be a risk to game stability, as HeavyStrategy 3 spawns ignore all spawn limits. To avoid this risk, make sure each Strategy reset timer is a little longer than all `Duration` HeavyStrategy 3 VltEd parameters.
...
* You can disable any feature of this set by deleting the file containing its parameter group. This does not apply to the engagement-count fix, which is tied to this entire feature set.
